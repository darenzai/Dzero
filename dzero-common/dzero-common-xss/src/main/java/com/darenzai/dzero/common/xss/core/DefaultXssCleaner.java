...
package com.darenzai.dzero.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.darenzai.dzero.common.xss.config.DzeroXssProperties;
import com.darenzai.dzero.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final DzeroXssProperties properties;

	public DefaultXssCleaner(DzeroXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(DzeroXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		DzeroXssProperties.Mode mode = properties.getMode();
		if (DzeroXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (DzeroXssProperties.Mode.validate == mode) {
			// 校验
